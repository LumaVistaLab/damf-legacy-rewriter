...
每个输入 bed 内的声道 ID 必须构成一段连续的数字范围。bed 可以从任意未占用 ID 开始；小于 7.1.2 的布局不再预留空槽位。

例如，5.1 side 和 5.1 back 输入都使用 6 个连续 ID。由声道标签而不是 ID 中的空洞决定第 4、5 个输入声道填入输出声床的 side 还是 rear 声道。

输入 bed 与对象 ID 可以相连，也可以留有间隔，还可以在整个源 ID 范围内交错。所有源声道 ID 和对象 ID 仍必须唯一。源 CAF 声道按所有已声明 bed/对象 ID 的升序对应，未声明的 ID 空洞不占 CAF 声道。

输出 ID 与上述输入映射相互独立：输出 bed 声道仍使用规范 7.1.2 槽位 ID，不压缩编号；输出对象声道 ID 从 `10` 开始连续递增。
...
输入 CAF 声道顺序由所有已声明输入 bed 和对象 ID 的升序确定。因此既支持对象位于两个 bed ID 范围之间，也支持 bed 与相邻对象之间存在 ID 空洞。
